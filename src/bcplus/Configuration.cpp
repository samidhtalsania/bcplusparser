/* Generated by re2c 0.13.5 on Fri Mar 28 21:54:02 2014 */
...
int Configuration::load(int argc, char const** argv) {
	...
	for (int i = 1; i < argc; i++) {
		if ((ret = parseOption(argv[i])) == Option::STAT_HELP 
			|| ret == Option::STAT_VERSION || ret == Option::STAT_BAD_ARG) return ret;
...
int Configuration::parseOption(char const* opt) {
	int ret = Option::STAT_OK;
	std::string val, val2;
    char const* marker;
...
#line 120 "bcplus/Configuration.r2c"
		{ 
...
yy13:
		++opt;
#line 116 "bcplus/Configuration.r2c"
		{ 
												  val = token;
...
yy15:
		++opt;
#line 112 "bcplus/Configuration.r2c"
		{ 
												  val = token;
												  option =  Option::HELP;
												  goto re2c_finish; }
...
yy18:
		++opt;
#line 87 "bcplus/Configuration.r2c"
		{ 
...
yy36:
#line 107 "bcplus/Configuration.r2c"
		{ 
												  val.assign(token, (size_t)(opt - token - 2));
												  val = "true"; 
...
yy37:
		++opt;
#line 102 "bcplus/Configuration.r2c"
		{ 
												  val.assign(token, (size_t)(opt - token - 2));
												  val = opt; 
												  option = Option::PARSE_TRACE;
...
yy43:
		++opt;
#line 97 "bcplus/Configuration.r2c"
		{ 
												  val.assign(token, (size_t)(opt - token - 2));
												  val2 = opt; 
...
yy58:
		++opt;
#line 92 "bcplus/Configuration.r2c"
		{ 
												  val.assign(token, (size_t)(opt - token - 2));
												  val2 = opt; 
...
re2c_finish:
	...
	case Option::HELP:
		ret = Option::STAT_HELP;
		break;
	case Option::VERSION:
		ret = Option::STAT_VERSION;
		break;
	case Option::BAD:			
	default:
		ret = Option::STAT_BAD_ARG;
		std::cerr << "ERROR: Unrecognized option: \"" << token << "\"." << std::endl;
		break;
	}
	return ret;
...
std::ostream& Configuration::outputHelp(std::ostream& out) const {
	out << "---------------------------------------------------------------------------------------"<< std::endl
		...
		<< "---------------------------------------------------------------------------------------"<< std::endl
		<< "Usage: " << *binaryName() << " [<OPTIONS>] [<CONSTANTS>] <INPUT_FILES>" 				<< std::endl
		<< "---------------------------------------------------------------------------------------"<< std::endl
		<< "OPTIONS:"																				<< std::endl;
	outputOptions(out);
	out 																							<< std::endl
